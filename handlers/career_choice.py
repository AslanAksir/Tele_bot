...
@router.message(Command(commands=['Профессия','prof']))
@router.message(F.text.lower() == 'профессия')
...
async def grade(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(f'Попробуй еще раз, {message.from_user.full_name}!', reply_markup=keyboard)
